...
<div class="container-fluid my-3">
    <form action="{{ route('donation.update', $donation) }}" method="POST">
        @csrf
        <div class="row g-4">
            <div class="col-md-8 col-12">
                <div class="card table-card">
                    ...
                    <div class="card-body custom-form">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <label for="donation_fund_id" class="form-label custom-label">Donation Fund</label>
                                <select class="form-select custom-input" id="donation_fund_id" name="donation_fund_id">
                                    <option value="">Select Donation Fund</option>
                                    @foreach ($funds as $fund)
                                        <option value="{{ $fund->id }}" {{ $donation->donation_fund_id == $fund->id ? 'selected' : '' }}>{{ $fund->name }}</option>
                                    @endforeach
                                </select>
                                @if($errors->has('donation_fund_id'))
                                    <div class="error_msg">{{ $errors->first('donation_fund_id') }}</div>
                                @endif
                            </div>

                            <div class="col-md-6 col-12">
                                <label for="payment_method" class="form-label custom-label">Payment Method</label>
                                <select class="form-select custom-input" id="payment_method" name="payment_method">
                                    <option value="">Select Payment Method</option>
                                    <option value="Islami-Bank" {{ $donation->payment_method == 'Islami-Bank' ? 'selected' : '' }}>Islami Bank Bangladesh Ltd</option>
                                    <option value="Bkash" {{ $donation->payment_method == 'Bkash' ? 'selected' : '' }}>Bkash</option>
                                    <option value="Nagad" {{ $donation->payment_method == 'Nagad' ? 'selected' : '' }}>Nagad</option>
                                </select>
                                @if($errors->has('payment_method'))
                                    <div class="error_msg">{{ $errors->first('payment_method') }}</div>
                                @endif
                            </div>
                            ...
                            <div class="col-md-6 col-12">
                                ...
                                <input type="text" id="transaction_id" class="form-control custom-input" name="transaction_id" value="{{ $donation->transaction_id }}">
                                @if($errors->has('transaction_id'))
                                    <div class="error_msg">{{ $errors->first('transaction_id') }}</div>
                                @endif
                            </div>

                            <div class="col-md-6 col-12">
                                <label for="amount" class="form-label custom-label">Amount</label>
                                <input type="number" id="amount" class="form-control custom-input" name="amount" value="{{ $donation->amount }}">
                                @if($errors->has('amount'))
                                    <div class="error_msg">{{ $errors->first('amount') }}</div>
                                @endif
                            </div>
